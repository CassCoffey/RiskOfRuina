...
{
    class EGOAirBackFallingAttack : BaseSkillState
    {
		...
		public override void OnEnter()
        {
			...
			base.PlayCrossfade("FullBody, Override", "EGOAirBackSlashContinue", "BaseAttack.playbackRate", this.duration, 0.1f);
        }
		...
		public override void FixedUpdate()
		{
			...
			} else
            {
				...
				if ((base.fixedAge >= this.duration || base.characterMotor.isGrounded))
				{
					...
					base.PlayCrossfade("FullBody, Override", "EGOAirBackSlashFinish", "BaseAttack.playbackRate", this.cooldown, 0.1f);
					...
					if (base.isAuthority)
                    {
						...
						effectData.scale = 1;
						EffectManager.SpawnEffect(this.GetComponent<RedMistStatTracker>().groundPoundEffect, effectData, false);
